...
- [firmware](#firmware)
  ...
  - [Getting Started](#getting-started)
    - [FastLED \& 8x8 WS2812B matrix (connected to D2)](#fastled--8x8-ws2812b-matrix-connected-to-d2)
...
### NeoPixel & 8x8 WS2812B matrix (connected to D2)

If you're using an 8x8 WS2812B (NeoPixel) matrix wired to the device's D2 pin, follow these steps:

- Install the Particle Neopixel (Adafruit_NeoPixel) library for Device OS:
...
- Wiring notes:
  - Connect the matrix DIN to the device D2 pin (LED data pin defined as `D2`).
  - Connect the matrix GND to the device GND (common ground).
  - Power the matrix with a suitable 5V supply capable of the required current (do not power many LEDs from the Particle device's 3V3 regulator).
  - For reliability, add a large electrolytic capacitor (1000uF, 6.3V+) across 5V and GND at the LED power input, and a small (300-500 ohm) resistor in series with DIN.

This project includes example code in `src/firmware.cpp` that initializes an Adafruit_NeoPixel strip for an 8x8 WS2812B matrix on `D2` and runs a simple rainbow animation. After adding the `neopixel` library, build and flash the firmware using Workbench or the Particle CLI.
